...
export function makeQuoteRoutes() {
    ...
    router.post("/", authMiddleware.authenticate(), (req, res) => new CreateQuoteController(quoteRepository, productRepository).handle(req, res));
    router.get("/", authMiddleware.authenticate(), (req, res) => new ListQuotesController(quoteRepository).handle(req, res));
    router.put("/:id", authMiddleware.authenticate(), (req, res) => new UpdateQuoteController(quoteRepository).handle(req, res));
    router.delete("/:id", authMiddleware.authenticate(), (req, res) => new DeleteQuoteController(quoteRepository).handle(req, res));
